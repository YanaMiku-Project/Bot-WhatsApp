exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         let caption = `彡  *G E T - A P K M O D*\n\n`
         ...
         caption += `    ◦  *Mod* : ${json.data.mod}\n`
         caption += `    ◦  *Price* : ${json.data.price}\n`
         ...
         if (command == 'getmod' || command == 'getapkmod') {
            if (json.file.url) {
               await client.sendFile(m.chat, json.file.url, json.file.filename || 'apkmod.apk', caption, m)
               client.sendReact(m.chat, '✅', m.key)
            }
         }
      } catch (e) {
